...
var VisitorListDetailsPage = /** @class */ (function () {
    function VisitorListDetailsPage(postPvd, router, actRoute, modalController, alertController, toastController, platform, document, file, fileOpener, transfer) {
    ...
    VisitorListDetailsPage.prototype.downloadGAF = function (name) {
        var _this = this;
        var fileTransfer = this.transfer.create();
        var url = 'https://www.azure-connect.com/pdf/' + name;
        this.file.checkDir(this.file.externalRootDirectory, 'downloads')
            .then(
        // Directory exists, check for file with the same name
        function (_) { return _this.file.checkFile(_this.file.externalRootDirectory, 'downloads/' + name)
            .then(function (_) {
            alert("A file with the same name already exists!");
        })
            // File does not exist yet, we can save normally
            .catch(function (err) {
            return fileTransfer.download(url, _this.file.externalRootDirectory + '/downloads/' + name).then(function (entry) {
        ...
        function (err) { return _this.file.createDir(_this.file.externalRootDirectory, 'downloads', false)
            .then(function (response) {
            alert('New folder created:  ' + response.fullPath);
            fileTransfer.download(url, _this.file.externalRootDirectory + '/downloads/' + name).then(function (entry) {
    ...
    VisitorListDetailsPage.prototype.getMimeByExt = function (name) {
