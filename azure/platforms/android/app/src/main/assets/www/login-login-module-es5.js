...
var LoginPage = /** @class */ (function () {
    function LoginPage(postPvd, router) {
        ...
        return new Promise(function (resolve) {
            ...
            _this.postPvd.postData(body, 'https://www.asi-ph.com/sandboxes/testAndroid/CondoProcess/').subscribe(function (data) {
                ...
                if (_this.Status == "Success") {
                    if (_this.typeData == "Owner") {
                        _this.router.navigateByUrl('/tabs/tab1', { state: { typeCode: _this.typeData, uCode: data['dataX'][0]['unit_code'], proCode: data['dataX'][0]['property_code'], hasTenant: data['hasTenant'], newCode: data['dataX'][0]['new_code'], fullname: data['dataX'][0]['fullname'], TUN: data['TUN'] } });
                    }
                    else if (_this.typeData == "Tenant") {
                        _this.router.navigateByUrl('/tabs/tab1', { state: { typeCode: _this.typeData, uCode: data['dataX'][0]['unit_code'], proCode: data['propC']['property_code'], hasTenant: data['hasTenant'], newCode: data['dataX'][0]['new_code'], fullname: data['dataX'][0]['fullname'], TUN: data['TUN'] } });
                    }
                }
            });
        });
